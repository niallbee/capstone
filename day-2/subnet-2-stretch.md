...
- Connecting to the database via private IPs
...
4. We now want to add our instance group to a backend service. In `webserver.tf` insert the following code block
   ...
   ```
5. To ensure that our webservers arer able to recieve the HTTP traffic we want to create a health check. Health checks regularly poll instances to ensure that they are able to recieve traffic by sending health probes over a designated port. If the health probe doesn't reach the instance successfully the instance is marked as "unhealthy" and traffic from the load balancer will not be sent to tht instance. To create the health check insert the following code block in `webserver.tf`
   ```
...
6. To allow the health probes to reach our VMs we need to create a firewall rule that allows them to reach the health check port. Insert the following code block into `netowrks.tf`
   ```
...
A NAT (Network Address Translation) router allows devices without a public IP to access the internet. Instaces that only have private IPs 
...
1. To create the NAT IP that our VMs will use insert the following code block into `networks.tf`
   ```
   ...
   ```
2. To create the router that will direct the traffic from our VMs to the internet via the NAT IP instert the following code blocks into `networks.tf`
   ```
...
Now that we have set up our backend service and its connectivity to the internet we need to introduce a load balancer. Because our VMs only have private IPs we need to provide an external IP in the form of a load balancer. For our purposes we are going to use a global external HTTP load balancer. 
...
2. So that we have easy access to the IP of our load balancer to access our website we can create an output. In `output.tf` insert the following code block
   ```
   output "load_balancer_ip" {
     value = google_compute_global_forwarding_rule.forwarding_rule.ip_address
   }
   ```
   Our VMs no longer have a public IP and we are going to reconfigure our database to use the private IP and remove the external so we can remove these outputs. In `outputs.tf` remove the `webserver_ip` and `database_ip` code blocks.
...
1. To create a private IP within our VPC for our database insert the following code blocks into `networks.tf`
   ...
   ```
2. We then need to establish a private connection between our private IP and the service producer's network. We can do this with a VPC network peering. Insert the following code block into `networks.tf`
   ```
   ...
   ```
3. Now that we have created a private connection to the service we need to configure the database to use the private IP. In `database.tf` replace the `ip_configuration` in the `google_sql_database_instance` resource block with the following
   ```
    ip_configuration {
      ipv4_enabled    = "false"
